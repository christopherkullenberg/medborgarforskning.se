...
class Project(models.Model):
    ...
    A note for PostgreSQL: the max_length of each entry may need to be increased
    for this model to work properly.
    '''
    ...
    name = models.CharField(help_text=_('Common name for the project.'), db_index=True, max_length=2000, blank=False,) ## maps to PPSR PMM  PMM projectName

    aim = models.CharField(help_text=_('Primary focus of the project.'), db_index=True, max_length=6000, default='', blank=False,) ## maps to PPSR PMM  projectAim
    description = models.CharField(max_length=50000, default=_(''), blank=False,) ## maps to PPSR PMM  projectDescription
    #status = models.ForeignKey(Status, on_delete=models.CASCADE)
    status = models.CharField(db_index=True, max_length=3000, default='0', choices=STATUS_CHOICES,) ## maps to PPSR PMM  projectStatus
    ...
    responsible_party_name = models.CharField(blank=True, max_length=500,) ## maps to PPSR PMM projectResponsiblePartyName
    responsible_party_email = models.CharField(blank=True, max_length=500,) ## maps to PPSR PMM projectResponsiblePartyContactEmail

    #contactPoint ## maps to PPSR PMM contactPoint - vocabulary not yet finalized by PPSR group
    contact_name = models.CharField(help_text=_('Name of the person maintaining this entry.'),max_length=2000,default='Not Provided', blank=True,) ## maps to PPSR PMM contactName

    #PPSR PMM meansOfContact - controlled vocabulary list ie (email, phone, website, physical address)
    contact_role = models.CharField(help_text=_('Maintainer’s role or position with this project.'), max_length=2000, default='Not Provided', blank=True,)
    ...
    contact_email = models.CharField(max_length=2000, default='',)
    contact_phone = models.CharField(max_length=2000, default='',)
    target_audience = models.CharField(db_index=True, help_text=_('Project primary audience.'), max_length=5000, default='',)
    #origin = models.CharField(max_length=100)
    image_dir = models.CharField(max_length= 2000,default= '',)
    ...
    exchanged = models.CharField(max_length=300, default='0', choices=SHARING_CHOICES,)
    ...
    image = models.URLField(max_length=5000, blank=True, verbose_name="Project url") # remote storage
